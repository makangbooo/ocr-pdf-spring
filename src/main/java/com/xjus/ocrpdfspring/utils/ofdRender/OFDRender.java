...
public class OFDRender {
    ...
    public static byte[] convertPdfToOfd(byte[] pdfBytes) {
        ...
        try {
            ...
            start = System.currentTimeMillis();
            com.xjus.ocrpdfspring.utils.ofdRender.OFDCreator ofdCreator = new com.xjus.ocrpdfspring.utils.ofdRender.OFDCreator();
            for (int i = 0; i < doc.getNumberOfPages(); i++) {
                ofdCreator.addPage(i);
                PDRectangle cropBox = doc.getPage(i).getCropBox();
                float widthPt = cropBox.getWidth();
                float heightPt = cropBox.getHeight();
                com.xjus.ocrpdfspring.utils.ofdRender.OFDPageDrawer ofdPageDrawer = new com.xjus.ocrpdfspring.utils.ofdRender.OFDPageDrawer(i, doc.getPage(i), ofdCreator, 1 / POINTS_PER_MM);
                ofdPageDrawer.drawPage();
                ofdCreator.addPageContent(i, ofdPageDrawer.getCtLayer(), widthPt / POINTS_PER_MM, heightPt / POINTS_PER_MM);
            }
            end = System.currentTimeMillis();
